# Log Rotation — `application2`

This section documents the automated log rotation setup for `application2`, managed by the orchestrator script at `/opt/EVA7_IMP/log_rotate.sh`.
...
1. **Structured logging initialised** — Every event the script emits is written as a JSON object (`timestamp`, `level`, `message`) to both stdout and the central log at `/opt/EVA7_IMP/log_orchestrator.log`. This makes log output machine-parseable and easy to forward to log aggregators.
...
3. **Lock acquisition** — A file lock is placed on `/opt/EVA7_IMP/log_orchestrator.lock` using `flock -n` (non-blocking, exclusive). If another instance of the script is already running, the new invocation logs a `WARN` and exits cleanly rather than racing. The lock is released automatically when the script exits.

4. **Config validation** — The script confirms that the `logrotate` config at `/etc/logrotate.d/application2` actually exists before proceeding. A missing config exits with `FATAL`.

5. **Output directory creation** — `/opt/EVA7_IMP/apps/` is created with `mkdir -p` if it does not already exist, preventing `logrotate` from failing on wildcard path expansion.

6. **`logrotate` execution** — `logrotate` runs with a dedicated state file (`/opt/EVA7_IMP/hourly.state`) so its rotation bookkeeping is isolated from the system-wide state. `stdout` and `stderr` are captured together. On success, an `INFO` entry is logged. On failure, the raw output is sanitised (double-quotes replaced with single-quotes to preserve JSON validity) and logged as `ERROR`, and the script exits non-zero.
...
Retention behaviour is controlled by `/etc/logrotate.d/application1` (shown below as a reference; `application2` should mirror this policy unless configured differently).

```
/opt/logs/application1.log {
...
| Live writer safety | `copytruncate` means `application2` never needs to be signalled or restarted during rotation |
...
| State isolation | The `-s /opt/EVA7_IMP/hourly.state` flag passed at runtime keeps this job's rotation state separate from `/var/lib/logrotate/status`, so it does not interfere with other system-managed logs |
...
```cron
0 0 * * * /opt/EVA7_IMP/log_rotate.sh
```
...
```bash
sudo /sbin/logrotate --debug -s /opt/EVA7_IMP/hourly.state /etc/logrotate.d/application2
```
...
```bash
sudo /sbin/logrotate --force -s /opt/EVA7_IMP/hourly.state /etc/logrotate.d/application2
```
...
```bash
sudo /opt/EVA7_IMP/log_rotate.sh
```

Then inspect the structured output log:

```bash
cat /opt/EVA7_IMP/log_orchestrator.log | python3 -m json.tool
# or, if jq is available:
cat /opt/EVA7_IMP/log_orchestrator.log | jq .
```
...
```bash
grep application2 /opt/EVA7_IMP/hourly.state
```
...
flock /opt/EVA7_IMP/log_orchestrator.lock sleep 30
